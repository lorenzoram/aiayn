...
def build_dataset_from_local():
    data = pd.read_csv('spa.csv', sep='\t', header=None)
    data.columns = ['en', 'es', 'attr']

    # Let's drop the 'attr' column if it's not needed
    data = data[['en', 'es']]

    # Save it to a CSV file without the header for compatibility with Hugging Face datasets
    data.to_csv('prepared_spa.csv', index=False, header=False)

    dataset = load_dataset('csv', data_files='prepared_spa.csv', split='train', column_names=['en', 'es'])
...
def get_dataset(config):
    ds_raw = build_dataset_from_local()
    ...
    val_data_loader = DataLoader(val_ds, batch_size=1, shuffle=False)
